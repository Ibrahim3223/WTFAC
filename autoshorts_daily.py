# autoshorts_daily.py — A/V hard lock + robust TTS + safe mux
# -*- coding: utf-8 -*-
...
def ffprobe_streams(p) -> str:
    try:
        return run(["ffprobe","-hide_banner","-loglevel","error","-show_streams","-select_streams","a:0","-of","json", p]).stdout
    except: return ""
...
def _rate_to_atempo(rate_str: str, default: float = 1.10) -> float:
    try:
        ...
        if rs.endswith(("x","X")): return max(0.5, min(2.0, float(rs[:-1])))
...
def tts_to_wav(text: str, wav_out: str) -> float:
    ...
    for attempt in range(2):
        try:
            ...
            _convert_mp3_to_wav(); pathlib.Path(mp3).unlink(missing_ok=True)
            d = ffprobe_dur(wav_out) or 0.0
            if d < 1.2: raise RuntimeError(f"TTS too short ({d:.2f}s)")
            return d
        except WSServerHandshakeError as e:
            if getattr(e, "status", None) == 401 or "401" in str(e):
                print("⚠️ edge-tts 401 → fallback TTS")
                break
            print(f"⚠️ edge-tts deneme {attempt+1} hata: {e}"); time.sleep(0.6)
    ...
    try:
        ...
        _convert_mp3_to_wav(); pathlib.Path(mp3).unlink(missing_ok=True)
        d = ffprobe_dur(wav_out) or 0.0
        if d < 1.2: raise RuntimeError(f"Fallback TTS too short ({d:.2f}s)")
        return d
    except Exception as e2:
        print(f"❌ Tüm TTS yolları hata: {e2} — sessiz üretmeyeceğim.")
        raise
...
def make_segment(src: str, dur_s: float, outp: str):
    ...
    run([
        ...
        "-vf", vf, "-r", str(TARGET_FPS), "-vsync","cfr","-an",
...
def enforce_video_exact_frames(video_in: str, target_frames: int, outp: str):
    ...
    run([
        "ffmpeg","-y","-hide_banner","-loglevel","error","-i", video_in,
...
def pad_video_to_duration(video_in: str, target_sec: float, outp: str):
    ...
    run([
        ...
        "-map","[v]","-r", str(TARGET_FPS), "-vsync","cfr",
        "-c:v","libx264","-preset","medium","-crf",str(CRF_VISUAL),
        "-pix_fmt","yuv420p","-movflags","+faststart", outp
    ])
...
def concat_videos_filter(files: List[str], outp: str):
    ...
    run([
        ...
        "-filter_complex", filtergraph, "-map","[v]", "-r", str(TARGET_FPS), "-vsync","cfr",
        "-c:v","libx264","-preset","medium","-crf",str(CRF_VISUAL),
        "-pix_fmt","yuv420p","-movflags","+faststart", outp
    ])
...
def lock_audio_duration(audio_in: str, target_frames: int, outp: str):
    ...
    run([
        "ffmpeg","-y","-hide_banner","-loglevel","error","-i", audio_in,
        "-af", f"atrim=end={dur:.6f},asetpts=N/SR/TB,aresample=48000",
        "-ar","48000","-ac","1","-c:a","pcm_s16le", outp
    ])

def _maybe_boost_or_fail(audio_in: str):
    # sessizlik veya çok düşük seviye tespiti
    probe = run([
        "ffmpeg","-hide_banner","-loglevel","error","-i", audio_in,
        "-af","astats=metadata=1:reset=1,ametadata=print:key=lavfi.astats.Overall.RMS_level:payload=1",
        "-f","null","-"
    ], check=False).stderr
    # ortalama seviye çıkar
    lvl = None
    for line in probe.splitlines():
        if "RMS_level" in line:
            try:
                lvl = float(line.split("RMS_level=")[-1]); break
            except: pass
    if lvl is None or lvl < -38.0:
        # otomatik gain uygula
        loud = audio_in.replace(".wav",".loud.wav")
        run(["ffmpeg","-y","-hide_banner","-loglevel","error","-i", audio_in,
             "-af","loudnorm=I=-16:TP=-1.5:LRA=11","-ar","48000","-ac","1","-c:a","pcm_s16le", loud])
        pathlib.Path(audio_in).unlink(missing_ok=True)
        pathlib.Path(loud).rename(audio_in)

def mux(video: str, audio: str, outp: str):
    # Video kopyalamak yerine yeniden kodla → PTS drift yok
    run([
        "ffmpeg","-y","-hide_banner","-loglevel","error",
        "-i", video, "-i", audio,
        "-map","0:v:0","-map","1:a:0",
...
def pexels_pick_one(query: str) -> Tuple[Optional[int], Optional[str]]:
    ...
    try:
        ...
        data = r.json() or {}; cand = []; block = _blocklist_get_pexels()
...
def main():
    ...
    # İçerik (kısaltılmış – önceki sürümle aynı mantık)
    if USE_GEMINI and GEMINI_API_KEY:
        ...
        for _ in range(8):
            try:
                ...
                print(f"Gemini error: {str(e)[:160]}"); time.sleep(0.8)
        if chosen is None:
            if last: ctry, tpc, sents, search_terms, ttl, desc, tags = last
            else:
                ...
                search_terms = ["cappadocia underground", "ancient tunnels", "turkey caves 4k"]; ttl = desc = ""; tags = []
    else:
        ctry, tpc = "World", "Amazing Facts"
        sents = [
            "Did you know Turkey hides a city underground?",
    ...
    # 3) Pexels & segment
    per_scene_queries = build_per_scene_queries([m[0] for m in metas], search_terms or [], MODE, topic=tpc)
    ...
    if len(clips) < len(metas): print("⚠️ Yeterli klip yok; mevcut klipleri döndürerek kullanılacak.")
