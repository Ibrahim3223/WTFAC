...
# Kokoro TTS (primary) • Whisper-timed ASS subtitles • Music mood + sidechain ducking • Soft xfade intro
# -*- coding: utf-8 -*-
...
# ---- TTS backends
# Preferred: Kokoro via local HTTP (set KOKORO_URL) or (best effort) kokoro-onnx; falls back to Edge-TTS, then gTTS.
TTS_ENGINE     = os.getenv("TTS_ENGINE", "kokoro").lower()      # kokoro | edge
KOKORO_URL     = os.getenv("KOKORO_URL", "").strip()            # e.g. http://localhost:8000/tts  (expects POST {text, voice} -> audio bytes)
KOKORO_VOICE   = os.getenv("KOKORO_VOICE", "af_sky")            # pick any voice your Kokoro server/model supports

# ---- Subtitles
SUBTITLES_MODE = os.getenv("SUBTITLES_MODE", "ass").lower()     # ass | drawtext
WHISPER_MODEL  = os.getenv("WHISPER_MODEL", "small")            # tiny/base/small/medium; small is CPU-friendly
...
def _tts_kokoro_http(text: str, tmp_out: str) -> bool:
    ...
    try:
        ...
        tmp.write_bytes(raw)
        # We don't know format; normalize via ffmpeg to WAV 48k mono
...
def tts_to_wav(text: str, wav_out: str) -> float:
    ...
    # Normalize with light dynamics & speed-up for Shorts cadence
    ...
    # 2) Edge-TTS → mp3 then wav (fallback)
    if not used_engine:
        ...
        if ok:
            run([
                ...
                "-i", mp3, "-ar","48000","-ac","1","-acodec","pcm_s16le", tmp_in
            ])
...
def _content_score(sentences: List[str]) -> float:
    ...
    return max(0.0, 10.0 - (bad * 1.4))  # 10 good, 0 bad
...
def transcribe_wav(wav_path: str, lang_hint: str = None):
    model = _whisper_model()
    language = lang_hint if (lang_hint and len(lang_hint)==2) else "en"
    segments, _ = model.transcribe(wav_path, language=language, vad_filter=True, vad_parameters=dict(min_silence_duration_ms=180))
    out=[]
    for seg in segments:
        out.append((float(seg.start), float(seg.end), clean_caption_text(seg.text)))
    return out

def make_ass_from_segments(segments, ass_path: str, style_overrides: dict=None):
    font = _ff_sanitize_font(font_path())
    style = f"Style: Default,{pathlib.Path(font).name if font else 'Arial'},42,&H00FFFFFF,&H000000FF,&H00000000,&H64000000,0,0,0,0,100,100,0,0,1,2,2,8,20,20,24,1"
    ...
    for (st,en,txt) in segments:
        if en <= st: en = st + 0.01
        lines.append(f"Dialogue: 0,{ts(st)},{ts(en)},Default,,0,0,0,,{txt}")
    pathlib.Path(ass_path).write_text(header+style+"\n\n"+events+"\n".join(lines), encoding="utf-8")
...
# =============================================================================
# Music selection + sidechain ducking
# =============================================================================
...
def main():
    ...
    if len(pool) < len(metas):
        extras = [
            "macro detail","city timelapse","nature macro","clean interior","close up hands",
            "ocean waves","night skyline","forest path","nature","globe","space","ocean"
        ]
    ...
    if not pool:
        raise RuntimeError("Pexels: no suitable clips found.")
    ...
    for i, ((base_text, d), src) in enumerate(zip(metas, chosen_files)):
        ...
        if SUBTITLES_MODE == "ass":
            ...
            words = transcribe_wav(wavs[i], lang_hint=LANG)
            if not words:
                words = [(0.0, d, base_text)]
            make_ass_from_segments(words, seg_ass, style_overrides=None)
    ...
    # 4.5) Optional xfade only for the first transition (keeps architecture simple)
    ...
    if USE_MUSIC:
        ...
        if music:
            ...
            print("🎵 No music file found, skipping music mix.")
