...
{{
    ...
    "confidence": 0.85
            ...
            logger.warning(f"⚠️ AI recommendation failed: {e}")
            return None
    ...
    def apply_pacing_to_clips(
        ...
        if len(clip_paths) != len(pacing_plan.optimized_durations):
            logger.warning(f"⚠️ Clip count mismatch: {len(clip_paths)} clips vs {len(pacing_plan.optimized_durations)} durations")
            return clip_paths
        ...
        for i, (clip_path, target_duration) in enumerate(zip(clip_paths, pacing_plan.optimized_durations)):
            ...
            if result.returncode == 0:
                ...
                logger.warning(f"   ⚠️ Failed to pace clip {i+1}, using original")
